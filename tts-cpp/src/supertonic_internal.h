...
//
// QVAC-18605 follow-up — same-shape matmul on both backends:
// confirmed by inspection of the CPU custom-op fast path
// (`ggml_custom_4d(F32, x->ne[0], w->ne[0], …)` → `[L, OC]`) and
// the `conv1d_f32(K=1)` GPU fallback (`ggml_reshape_3d(result,
...
//
// History note: the original (PR #16) version of this helper
// assumed `q->ne[0] = HD` and `q->ne[1] = L` — i.e., the
// transpose of what the matmul actually produces.  That older
// contract crashed at the new defensive assertion below on every
// real synth (CPU, OpenCL, Vulkan) the moment a GGUF carrying
// `vector_rope_theta` enabled the in-graph rotation path.  The
// CPU unit test that landed alongside `apply_rope_to_packed_qk`
// hand-built Q under the `[HD, L]` assumption, so the failure
// mode was invisible until the first end-to-end synth attempt.
// `test_supertonic_rope_packed_qk.cpp` now reproduces the
// **production** matmul layout and pins both the input and
...
//     transpose).  On Vulkan that's one ~256-thread shader
//     dispatch per cache build; on CPU it's a single memcpy of
//     `L * HD * 4` bytes.  Within one synth's 5 denoise steps,
//     the cache is built ONCE and reused, so the cost is
//     amortised over all subsequent steps.
//   - Eliminates 40 CPU rotations / synth (~50 µs each ≈ 2 ms
//     wall-time on the default 5-step × 4-RoPE-site schedule).
//   - Net: the original rounds-8/9 GPU bridge wins are
//     preserved AND now actually run end-to-end without
//     crashing.
//
