...
// SFINAE: assert that `load_supertonic_gguf` accepts the new
// 7th argument (a const-ref vector of strings).  Same compile-
// time gate as above.
...
void test_load_supertonic_gguf_param_exists() {
    std::fprintf(stderr, "[Round 6 API: load_supertonic_gguf 7th param]\n");
    static_assert(
        decltype(has_deny_list_param_in_load<>(0))::value,
        "load_supertonic_gguf must accept an optional 7th f16_weights_deny_list parameter");
