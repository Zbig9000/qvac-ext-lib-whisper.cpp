// QVAC-18605 follow-up — Vulkan / OpenCL synth-path regression.
//
// Background
// ----------
...
// Production-layout parity test.
//
// Build q with `ne=[L, HD]` (matches `dense_matmul_time_ggml`
// output on every backend).  Memory layout is channel-major-flat
// `data[t + c*L]` per ggml's natural strides for that ne.
//
// Reference is built in time-major-flat layout (scalar
// apply_rope's natural index `data[t*HD + c]`); the upload
// transposes from time-major-flat to channel-major-flat so the
// graph input matches matmul's contract bit-for-bit.
//
// Apply the scalar reference IN-PLACE on the time-major-flat
// buffer, then compare to the helper's downloaded bytes.  The
// helper must produce bytes in time-major-flat layout so:
...
void test_production_layout(const char * label, int L, int n_heads, int head_dim,
    ...
    // explicitly DIFFERENT from the old test's ne=[HD, L].
